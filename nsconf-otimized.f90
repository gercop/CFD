...
ccccc **************************************************************************
     ...
     &  U5(local_imax,jmax,kmax)
      ...
      allocate(du_dx (local_imax,jmax,kmax),du_dy (local_imax,jmax,kmax),
...
ccccc ----------------------------------------------------
ccccc CALCULATING THE PRESSURE, TEMPERATURE AND VISCOSITY
ccccc ----------------------------------------------------
      allocate(p (local_imax,jmax,kmax),
     &         tp(local_imax,jmax,kmax),
     &         mu(local_imax,jmax,kmax))
      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            !MS$IF (tp_proc.EQ.1)
...
ccccc ----------------------------------------------------
ccccc CALCULATING THE SHEAR-STRESS TENSOR
ccccc ----------------------------------------------------
      allocate(tau_xx(local_imax,jmax,kmax),tau_yy(local_imax,jmax,kmax),
     &         tau_zz(local_imax,jmax,kmax),tau_xy(local_imax,jmax,kmax),
     &         tau_xz(local_imax,jmax,kmax),tau_yz(local_imax,jmax,kmax))
      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            tau_xx(i,j,k) = 2.d0/3.d0*mu(i,j,k)/Re*(2.d0*du_dx(i,j,k)-dv_dy(i,j,k)-dw_dz(i,j,k))
            tau_yy(i,j,k) = 2.d0/3.d0*mu(i,j,k)/Re*(2.d0*dv_dy(i,j,k)-du_dx(i,j,k)-dw_dz(i,j,k)) 
            tau_zz(i,j,k) = 2.d0/3.d0*mu(i,j,k)/Re*(2.d0*dw_dz(i,j,k)-du_dx(i,j,k)-dv_dy(i,j,k))
            tau_xy(i,j,k) = mu(i,j,k)/Re*(du_dy(i,j,k)+dv_dx(i,j,k))
            tau_xz(i,j,k) = mu(i,j,k)/Re*(du_dz(i,j,k)+dw_dx(i,j,k))
            tau_yz(i,j,k) = mu(i,j,k)/Re*(dv_dz(i,j,k)+dw_dy(i,j,k))
          end do
        end do
      end do    
      deallocate(mu,du_dx,du_dy,du_dz,dv_dx,dv_dy,dv_dz,dw_dx,dw_dy,dw_dz)

ccccc -------------------------------
ccccc CALCULATING THE FLUX VARIABLES
ccccc -------------------------------  
      allocate(F(local_imax,jmax,kmax),G(local_imax,jmax,kmax),H(local_imax,jmax,kmax))
      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            F(i,j,k) = U2(i,j,k)
      ...
      call calculating_flux(local_imax,U1_RK1) 

      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            F(i,j,k) = U2(i,j,k)**2.d0/U1(i,j,k)+p(i,j,k)-tau_xx(i,j,k)
            G(i,j,k) = U2(i,j,k)*U3(i,j,k)/U1(i,j,k)-tau_xy(i,j,k)
            H(i,j,k) = U2(i,j,k)*U4(i,j,k)/U1(i,j,k)-tau_xz(i,j,k)
          end do
        end do
      end do    
      call calculating_flux(local_imax,U2_RK1) 

      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            F(i,j,k) = U2(i,j,k)*U3(i,j,k)/U1(i,j,k)-tau_xy(i,j,k)
            G(i,j,k) = U3(i,j,k)**2.d0/U1(i,j,k)+p(i,j,k)-tau_yy(i,j,k) 
            H(i,j,k) = U3(i,j,k)*U4(i,j,k)/U1(i,j,k)-tau_yz(i,j,k)
          end do
        end do
      end do    
      call calculating_flux(local_imax,U3_RK1) 

      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            F(i,j,k) = U2(i,j,k)*U4(i,j,k)/U1(i,j,k)-tau_xz(i,j,k)
            G(i,j,k) = U3(i,j,k)*U4(i,j,k)/U1(i,j,k)-tau_yz(i,j,k)
            H(i,j,k) = U4(i,j,k)**2.d0/U1(i,j,k)+p(i,j,k)-tau_zz(i,j,k)
          end do
        end do
      end do    
      call calculating_flux(local_imax,U4_RK1) 

      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            F(i,j,k) = (U5(i,j,k)+p(i,j,k))*U2(i,j,k)/U1(i,j,k)
     &        -U2(i,j,k)/U1(i,j,k)*2.d0/3.d0*tau_xx(i,j,k)
     &        -U3(i,j,k)/U1(i,j,k)*tau_xy(i,j,k)
     &        -U4(i,j,k)/U1(i,j,k)*tau_xz(i,j,k)
     &        +lambda*dtp_dx(i,j,k)
            G(i,j,k) = (U5(i,j,k)+p(i,j,k))*U3(i,j,k)/U1(i,j,k)
     &        -U2(i,j,k)/U1(i,j,k)*tau_xy(i,j,k)
     &        -U3(i,j,k)/U1(i,j,k)*tau_yy(i,j,k)
     &        -U4(i,j,k)/U1(i,j,k)*tau_yz(i,j,k)
     &        +lambda*dtp_dy(i,j,k)
            H(i,j,k) = (U5(i,j,k)+p(i,j,k))*U4(i,j,k)/U1(i,j,k)
     &        -U2(i,j,k)/U1(i,j,k)*tau_xz(i,j,k)
     &        -U3(i,j,k)/U1(i,j,k)*tau_yz(i,j,k)
     &        -U4(i,j,k)/U1(i,j,k)*tau_zz(i,j,k)
     &        +lambda*dtp_dz(i,j,k)
          end do
        end do
      end do    
      !MS$IF (tp_proc.EQ.1)
        call calculating_flux(local_imax,U5_RK1) 
      !MS$ELSEIF (tp_proc.EQ.2)
        U5_RK1(:,:,:) = 0.d0
      !MS$ENDIF          

      deallocate(p,dtp_dx,dtp_dy,dtp_dz,tau_xx,tau_yy,tau_zz,
     &  tau_xy,tau_xz,tau_yz,F,G,H)
...
ccccc **************************************************************************
      SUBROUTINE calculating_flux(local_imax,U_RK)
      ...
      integer i,j,k,local_imax
      real*8 
     &  U_RK (local_imax,jmax,kmax),DF_dx(local_imax,jmax,kmax),
