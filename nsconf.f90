...
ccccc **********************************************************************
      ...
      real*8 t,dt2,dt6,x(local_imax),y(jmax),z(kmax),u_1,v_1,w_1,xx,yy,zz
...
ccccc **************************************************************************
     ...
     &  U5(local_imax,jmax,kmax),E (local_imax,jmax,kmax),
...
ccccc -------------------------------     
      ...
      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            E(i,j,k) = U2(i,j,k)**2.d0/U1(i,j,k)+p(i,j,k)-2.d0/3.d0*mu(i,j,k)/Re*
     &        (2.d0*du_dx(i,j,k)-dv_dy(i,j,k)-dw_dz(i,j,k))
            F(i,j,k) = U2(i,j,k)*U3(i,j,k)/U1(i,j,k)-mu(i,j,k)/Re*(du_dy(i,j,k)+dv_dx(i,j,k))
            G(i,j,k) = U2(i,j,k)*U4(i,j,k)/U1(i,j,k)-mu(i,j,k)/Re*(du_dz(i,j,k)+dw_dx(i,j,k))
          end do
        end do
      end do    
      call calculating_flux(local_imax,E,F,G,U2_RK1) 

      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            E(i,j,k) = U2(i,j,k)*U3(i,j,k)/U1(i,j,k)-mu(i,j,k)/Re*(du_dy(i,j,k)+dv_dx(i,j,k))
            F(i,j,k) = U3(i,j,k)**2.d0/U1(i,j,k)+p(i,j,k)-2.d0/3.d0*mu(i,j,k)/Re*(
     &        2.d0*dv_dy(i,j,k)-du_dx(i,j,k)-dw_dz(i,j,k)) 
            G(i,j,k) = U3(i,j,k)*U4(i,j,k)/U1(i,j,k)-mu(i,j,k)/Re*(dv_dz(i,j,k)+dw_dy(i,j,k))
          end do
        end do
      end do    
      call calculating_flux(local_imax,E,F,G,U3_RK1) 

      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            E(i,j,k) = U2(i,j,k)*U4(i,j,k)/U1(i,j,k)-mu(i,j,k)/Re*(du_dz(i,j,k)+dw_dx(i,j,k))
            F(i,j,k) = U3(i,j,k)*U4(i,j,k)/U1(i,j,k)-mu(i,j,k)/Re*(dv_dz(i,j,k)+dw_dy(i,j,k))
            G(i,j,k) = U4(i,j,k)**2.d0/U1(i,j,k)+p(i,j,k)-2.d0/3.d0*mu(i,j,k)/Re*(
     &        2.d0*dw_dz(i,j,k)-du_dx(i,j,k)-dv_dy(i,j,k))
          end do
        end do
      end do    
      call calculating_flux(local_imax,E,F,G,U4_RK1) 

      !MS$IF (tp_proc.EQ.1)
        do k=1,kmax 
          do j=1,jmax 
            do i=1,local_imax
              E(i,j,k) = (U5(i,j,k)+p(i,j,k))*U2(i,j,k)/U1(i,j,k)
     ...
     &          +lambda*dtp_dx(i,j,k)
              F(i,j,k) = (U5(i,j,k)+p(i,j,k))*U3(i,j,k)/U1(i,j,k)
     ...
     &          +lambda*dtp_dy(i,j,k)
              G(i,j,k) = (U5(i,j,k)+p(i,j,k))*U4(i,j,k)/U1(i,j,k)
     ...
     &          +lambda*dtp_dz(i,j,k)
        ...
        call calculating_flux(local_imax,E,F,G,U5_RK1) 
      !MS$ELSEIF (tp_proc.EQ.2)
        U5_RK1(:,:,:) = 0.d0
      !MS$ENDIF          
...
ccccc **************************************************************************
      SUBROUTINE calculating_flux(local_imax,E,F,G,U_RK)
     ...
     &  E    (local_imax,jmax,kmax),F    (local_imax,jmax,kmax),
     &  G    (local_imax,jmax,kmax),dE_dx(local_imax,jmax,kmax),
     &  dF_dy(local_imax,jmax,kmax),dG_dz(local_imax,jmax,kmax)

      call der_x_DD(my_rank,pro,local_imax,E,dE_dx)
      call der_y_WN(            local_imax,F,dF_dy)
      call der_z_PP(            local_imax,G,dG_dz)

      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            U_RK(i,j,k) = -dE_dx(i,j,k)-dF_dy(i,j,k)-dG_dz(i,j,k)
          end do
        end do
      end do	
...
ccccc **********************************************************************
     ...
     &              U2(i,j,k)**2.d0+U3(i,j,k)**2.d0+U4(i,j,k)**2.d0))
              ...
              U2(1,j,k) = rho_0(1,j,k)*(u_0(1,j,k)+u_1) 
              U3(1,j,k) = rho_0(1,j,k)*(v_0(1,j,k)+v_1)
              U4(1,j,k) = rho_0(1,j,k)*(          +w_1)
