...
ccccc **********************************************************************
     ...
     &  Qi_full,rho_full,p_full,tp_full,Et_full
      ...
      write(*,*) 
      write(*,*) ' CONVERTING FILES FROM BINARY FORMAT TO TECPLOT PATTERN '   
      write(*,*)      
            ...
            do extra_yy=1,extra_y    
              do extra_xx=1,extra_x              
                ...
                if (error.EQ.0) then
                  ...
                  if (extra_x.GT.1) x_adj = x_adj + x_full(imax) + (x_full(2)-x_full(1))
                  if (extra_y.GT.1) y_adj = y_adj + y_full(jmax) + (y_full(2)-y_full(1))
                  if (extra_z.GT.1) z_adj = z_adj + z_full(kmax) + (z_full(2)-z_full(1))
                end if
              end do 
            end do
          end do  
...
ccccc **********************************************************************
     ...
     &  extra_x*imax,extra_y*jmax,kmax,'BLOCK'//NULLCHR,NULLCHR)
      ...
      write(*,*) ' PASSOU AQUI WRITE DATA IN FILE: ',dirgraf//'f_'//fname
      write(*,*)
