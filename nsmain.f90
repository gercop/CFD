...
ccccc **********************************************************************
      ...
      real*8 x(local_imax),y(jmax),z(kmax),
     &  dt_max_1,dt_min_1,dt_max_2,u_m,v_m,w_m,dx_aux,dy_aux,dz_aux,
     ...
     &  w(local_imax,jmax,kmax)
      ...
      call maximo_abs_3D(local_imax,jmax,kmax,u,u_m) 
      call maximo_abs_3D(local_imax,jmax,kmax,v,v_m) 
      call maximo_abs_3D(local_imax,jmax,kmax,w,w_m)   
     ...
     &          1.d0/dz_aux**2.d0))
      ...
      !MS$IF (show_msg.EQ.1)
        if (my_rank.EQ.0) then
          write (*,*)
          write (*,*) 'DETAILS OF COMP. VELOCITIES : ' 
          !MS$IF (tp_dim.EQ.1).OR.(tp_dim.EQ.2).OR.(tp_dim.EQ.3)
            write (*,'(A37,f22.15)') ' MAXIMUM STREAMWISE COMP. VELOCITY...:',u_m
          !MS$ENDIF 
          !MS$IF (tp_dim.EQ.2).OR.(tp_dim.EQ.3)
            write (*,'(A37,f22.15)') ' MAXIMUM NORMALWISE COMP. VELOCITY...:',v_m
          !MS$ENDIF 
          !MS$IF (tp_dim.EQ.3)
            write (*,'(A37,f22.15)') ' MAXIMUM SPANWISE COMP. VELOCITY.....:',w_m
          !MS$ENDIF 
          write (*,*) 
          write (*,*) 'DETAILS CFL: '
          write (*,*) 'DT      = ',dt
          write (*,*) 'DT_MAX  = ',dt_min_1
          write (*,*) 'RATIO   = ',dt_min_1/dt 
        end if 
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      !MS$IF (tp_proc.EQ.2)
        if ((jmax.NE.1).AND.(kmax.NE.1)) then
          if ((alpha.NE.kapa).OR.(kapa.NE.beta).OR.(beta.NE.alpha)) then
            ...
            write(*,*) '-------------------------------------------'
            write(*,*) 'EXPLANATION: Analytical solutin for the    '
            write(*,*) 'three-dimensional acoustic wave problems   '
            write(*,*) 'with isentropic flow should be performed   ' 
            write(*,*) 'with the following parameters:             '
      ...
      !MS$IF (tp_proc.EQ.1)
        if (Pr.NE.1.d0) then
            ...
            write(*,*) 'PARAMETERS ERRORS...                       '
            write(*,*) '-------------------------------------------'
            write(*,*) 'EXPLANATION: To run the code with the      '
            write(*,*) 'analytical solution for Navier-Stokes of   '
            write(*,*) 'Crocco-Busemann is need setting Pr=1.       ' 
            write(*,*) '-------------------------------------------'
            write(*,*) 'WARNING: THE PROGRAMM WILL BE TERMINATE.   '
            write(*,*) '-------------------------------------------'
	    read(*,*)
     ...
     &      (beta.NE.2.d0*Pi).OR.(Pr.NE.1.d0)) then
          ...
          write(*,*) 'EXPLANATION: Analytical solutin for the    '
          write(*,*) 'three-dimensional acoustic wave problems   '
          write(*,*) 'with ful energy equation should be         ' 
          write(*,*) 'performed with the following parameters:   '
          write(*,*) 'alpha=kapa=beta;                           '
          write(*,*) 'Ma=1.0;                                    '
      ...
      !MS$IF (tp_s.EQ.2).OR.(tp_s.EQ.3).OR.(tp_s.EQ.4)
        !MS$IF (parallel.EQ.01)
          write(*,*)
          write(*,*) '------------------------------------------------'
          write(*,*) 'PARAMETERS ERRORS...                            '
          write(*,*) '------------------------------------------------'
          write(*,*) 'EXPLANATION: For acoutis and temporal shear     '
 	  write(*,*) 'layer problem the code has to work sequential   '
          write(*,*) 'computer proccess (parallel=0).                 '
          write(*,*) '------------------------------------------------'
          write(*,*) 'WARNING: THE PROGRAMM WILL BE TERMINATE!        '
          write(*,*) '------------------------------------------------'
          STOP
        !MS$ENDIF
        !MS$IF (stretching_in_x.EQ.01)
          write(*,*)
          write(*,*) '------------------------------------------------'
          write(*,*) 'PARAMETERS ERRORS...                            '
          write(*,*) '------------------------------------------------'
          write(*,*) 'EXPLANATION: For acoustic and temporal shear    '
 	  write(*,*) 'layer problem the code has to work with         '
          write(*,*) 'equidistant grid in the longitudinal direction  '
          write(*,*) '(stretching_in_x=0).                            '
          write(*,*) '------------------------------------------------'
          write(*,*) 'WARNING: THE PROGRAMM WILL BE TERMINATE!        '
          write(*,*) '------------------------------------------------'
          STOP
      ...
      !MS$IF (tp_s.NE.5).AND.(tp_sp.NE.6).AND.(parallel.EQ.1)
        if (M2.LT.0) then
          ...
          write(*,*) 'PARAMETERS ERRORS...                          '
          write(*,*) '----------------------------------------------'
          write(*,*) 'EXPLANATION: For spatial simulations the vel. '
          write(*,*) 'profile locate at y<0 should be positive.     '
          write(*,*) '----------------------------------------------'
          write(*,*) 'WARNING: THE PROGRAMM WILL BE TERMINATE.      '
          write(*,*) '----------------------------------------------'
	  read(*,*)
      ...
      !MS$IF (tp_dim.EQ.1)
        if ((kapa.NE.0).AND.(beta.NE.0)) then
          write(*,*)
          write(*,*) '------------------------------------------------'
          write(*,*) 'PARAMETERS ERRORS...                            '
          write(*,*) '------------------------------------------------'
        ...
        if ((jmax.GT.1).OR.(kmax.GT.1)) then
          write(*,*)
          write(*,*) '------------------------------------------------'
          write(*,*) 'PARAMETERS ERRORS...                            '
          write(*,*) '------------------------------------------------'
	  write(*,*) 'EXPLANATION: For 1D problem we most have jmax=1 '
          write(*,*) 'and kmax=1.                                       '
          write(*,*) '------------------------------------------------'
          write(*,*) 'WARNING: THE PROGRAMM WILL BE TERMINATE!        '
          write(*,*) '------------------------------------------------'
      ...
      !MS$ELSEIF (tp_dim.EQ.2)
        if (kapa.EQ.0) then
          ...
          write(*,*) 'PARAMETERS ERRORS...                            '
          write(*,*) '------------------------------------------------'
          write(*,*) 'EXPLANATION: For 2D problem we most have kapa>0 '
          write(*,*) '------------------------------------------------'
          write(*,*) 'WARNING: THE PROGRAMM WILL BE TERMINATE!        '
          write(*,*) '------------------------------------------------'
          STOP
        ...
        if (beta.NE.0) then
          ...
          write(*,*) 'PARAMETERS ERRORS...                            '
          write(*,*) '------------------------------------------------'
          write(*,*) 'EXPLANATION: For 2D problem we most have beta=0 '
          write(*,*) '------------------------------------------------'
          write(*,*) 'WARNING: THE PROGRAMM WILL BE TERMINATE!        '
          write(*,*) '------------------------------------------------'
          STOP
        ...
        if (kmax.GT.1) then
          ...
          write(*,*) 'PARAMETERS ERRORS...                            '
          write(*,*) '------------------------------------------------'
          write(*,*) 'EXPLANATION: For 2D problem we most have kmax=1.'
          write(*,*) '------------------------------------------------'
          write(*,*) 'WARNING: THE PROGRAMM WILL BE TERMINATE!        '
          write(*,*) '------------------------------------------------'
          STOP
...
ccccc **********************************************************************
     ...
     &  global_U1,global_U2,global_U3,global_U4,global_U5)
      ...
      call sv_SN_BIN(fnamegraf,x,y,z,tt,global_U1,global_U2,
