...
ccccc **********************************************************************
     ...
     &  lhs_d1_62th_D,lhs_d1_62th_N,lhs_d1_6th_P
      ...
      real*8 x(imax),y(jmax),z(kmax),t,cyy,py_h2,aa,bb,B1,B2,
...
cccccc ************************************
     ...
     &        *dsinh(sp_y*B)**2.d0)**(3.d0/2.d0) ) 
      ...
      !MS$ELSEIF (tp_s.EQ.6)
        ...
        !MS$ELSEIF (stretching_in_y.EQ.1)            
          !Obs: Para a malha do problema de camada limite 
          !     com estiramento na direção y foi adotado 
          !     o parâmetro st_y = 1.08d0.
          B1 = ((sp_y+1.d0)/(sp_y-1.d0))
          B2 = dlog(B1)
          
          do j=1,jmax 
            cyy       = dble(j-1)*dcy
            py_aux(j) = Ly*( (sp_y+1.d0)-(sp_y-1.d0)*(B1**(1.d0-cyy)) )
     &        /(B1**(1.d0-cyy)+1.d0)
