...
ccccc **********************************************************************
     ...
     &            dexp(-sigma*yy**2.d0)
            ...
            w (1,j,k)  = 0.d0        + 1.d0/(M1*c)*w_1
            tp(1,j,k)  = tp_0(1,j,k)     

            call extrapolated_inlet(local_imax,j,k,p)        !Pressure is extrapolated to satisfy dp_dx=0

            !MS$IF (tp_proc.EQ.1)
              rho(1,j,k) = (gamma*Ma**2.d0)*p(1,j,k)/tp(1,j,k)
            !MS$ELSEIF (tp_proc.EQ.2)
              rho(1,j,k) = (p(1,j,k)*gamma*Ma**2.d0)**(1.d0/gamma)
            !MS$ENDIF         
     ...
     &        u(1,j,k)**2.d0+v(1,j,k)**2.d0+w(1,j,k)**2.d0))
            ...
            !MS$IF (tp_proc.EQ.1)
              rho(local_imax,j,k) = (gamma*Ma**2.d0)*p(local_imax,j,k)/tp(local_imax,j,k)
            !MS$ELSEIF (tp_proc.EQ.2)
              rho(local_imax,j,k) = (p(local_imax,j,k)*gamma*Ma**2.d0)**(1.d0/gamma)
            !MS$ENDIF         
      ...
      !MS$IF (tp_s.EQ.6)
        do k=1,kmax
          do i=1,local_imax
            ...
            !MS$IF (tp_proc.EQ.1)
              rho(i,jmax,k) = (gamma*Ma**2.d0)*p(i,jmax,k)/tp(i,jmax,k)
            !MS$ELSEIF (tp_proc.EQ.2)
              rho(i,jmax,k) = (p(i,jmax,k)*gamma*Ma**2.d0)**(1.d0/gamma)
            !MS$ENDIF         
            e (i,jmax,k) = p(i,jmax,k)/(rho(i,jmax,k)*(gamma-1.d0)) 
            Et(i,jmax,k) = rho(i,jmax,k)*(e(i,jmax,k)+1.d0/2.d0*(
     &        u(i,jmax,k)**2.d0+v(i,jmax,k)**2.d0+w(i,jmax,k)**2.d0))             
          end do
        end do
      !MS$ENDIF       
