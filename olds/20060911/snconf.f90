...
ccccc **********************************************************************
      SUBROUTINE flux_to_primitive(local_imax,E1,E2,E3,E4,E5,rho,u,v,w,Et,p,tp,mu)
...
ccccc **********************************************************************
      SUBROUTINE RK4(local_imax,x,y,z,tt,E1,E2,E3,E4,E5)
     ...
     &  E1    (local_imax,jmax,kmax),E2    (local_imax,jmax,kmax),
     &  E3    (local_imax,jmax,kmax),E4    (local_imax,jmax,kmax),
     &  E5    (local_imax,jmax,kmax),E1_t  (local_imax,jmax,kmax), 
     &  E2_t  (local_imax,jmax,kmax),E3_t  (local_imax,jmax,kmax),
     &  E4_t  (local_imax,jmax,kmax),E5_t  (local_imax,jmax,kmax),
     &  E1_rk1(local_imax,jmax,kmax),E1_rk2(local_imax,jmax,kmax),
     &  E1_rk3(local_imax,jmax,kmax),E1_rk4(local_imax,jmax,kmax), 
     &  E2_rk1(local_imax,jmax,kmax),E2_rk2(local_imax,jmax,kmax),
     &  E2_rk3(local_imax,jmax,kmax),E2_rk4(local_imax,jmax,kmax), 
     &  E3_rk1(local_imax,jmax,kmax),E3_rk2(local_imax,jmax,kmax),
     &  E3_rk3(local_imax,jmax,kmax),E3_rk4(local_imax,jmax,kmax),
     &  E4_rk1(local_imax,jmax,kmax),E4_rk2(local_imax,jmax,kmax),
     &  E4_rk3(local_imax,jmax,kmax),E4_rk4(local_imax,jmax,kmax),
     &  E5_rk1(local_imax,jmax,kmax),E5_rk2(local_imax,jmax,kmax),
     &  E5_rk3(local_imax,jmax,kmax),E5_rk4(local_imax,jmax,kmax)
...
ccccc Mass Equation:
ccccc     
ccccc 	d rho	    d (rho u)	   d (rho v)
ccccc 	----- = -  ----------- -  ----------- 
ccccc 	 d t	       d x	      d y
ccccc --------------------------------------------------------------------------
ccccc Momentun Equation in the x direction:
ccccc                                              _                         _
ccccc d rho u     d rho u2   d rho u v   d p   1  |  4 d2 u   d2 u   1 d2 v   |
ccccc ------- = - -------- - --------- - --- + -- |  - ---- + ---- + - -----  |
ccccc   d t         d x         d y      d x   Re |_ 3 d x2   d y2   3 dx dy _|
ccccc --------------------------------------------------------------------------
ccccc Momentun Equation in the y direction:
ccccc                                              _                         _
ccccc d rho v     d rho u v   d rho v2   d p   1  |  4 d2 v   d2 v   1 d2 u   |
ccccc ------- = - --------- - -------- - --- + -- |  - ---- + ---- + - -----  |
ccccc   d t          d x        d y      d y   Re |_ 3 d y2   d x2   3 dx dy _|
ccccc --------------------------------------------------------------------------
ccccc Momentun Equation in the z direction:
ccccc                                              _                         _
ccccc d rho w     d rho u v   d rho v2   d p   1  |  4 d2 v   d2 v   1 d2 u   |
ccccc ------- = - --------- - -------- - --- + -- |  - ---- + ---- + - -----  |
ccccc   d t          d x        d y      d z   Re |_ 3 d y2   d x2   3 dx dy _|
ccccc
ccccc **************************************************************************
     ...
     &  E1_RK,E2_RK,E3_RK,E4_RK,E5_RK)
      ...
      integer i,j,k,local_imax
      real*8 t,x(local_imax),y(jmax),z(kmax),lambda,
     &  E1       (local_imax,jmax,kmax),E2       (local_imax,jmax,kmax),
     &  E3       (local_imax,jmax,kmax),E4       (local_imax,jmax,kmax),
     ...
     &  dw_dz    (local_imax,jmax,kmax),dp_dx    (local_imax,jmax,kmax),
     &  dp_dy    (local_imax,jmax,kmax),dp_dz    (local_imax,jmax,kmax),
     &  drho_dx  (local_imax,jmax,kmax),drho_dy  (local_imax,jmax,kmax),
     &  drho_dz  (local_imax,jmax,kmax),dEt_dx   (local_imax,jmax,kmax),     
     &  dEt_dy   (local_imax,jmax,kmax),dEt_dz   (local_imax,jmax,kmax),     
     ...
     &  du_0_dy  (local_imax,jmax,kmax),d2u_0_dy2(local_imax,jmax,kmax)
      ...
      call boundflow(local_imax,x,y,z,t,rho,u,v,w,Et,p,tp,mu)

ccccc ----------------------------------
ccccc CALCULATING THE CONVECTIVE TERM
ccccc ----------------------------------      
      call der_x_D (local_imax,u,  du_dx  )
      call der_x_D (local_imax,v,  dv_dx  )    
      call der_x_D (local_imax,w,  dw_dx  ) 
      call der_x_D (local_imax,rho,drho_dx)
      call der_x_D (local_imax,p,  dp_dx  )
      call der_x_D (local_imax,Et, dEt_dx )
      call der_x_D (local_imax,tp, dtp_dx )
      call der_x_D (local_imax,mu, dmu_dx )
      ...
      call der_z_P (local_imax,mu, dmu_dz )
...
ccccc ---------------------------------
     ...
     &            +d2w_dyz(i,1,k)) 
     &          +1.d0/Re*(
     &            +dmu_dx(i,j,k)*du_dy(i,1,k)
     &            +dmu_dx(i,j,k)*dv_dx(i,1,k)
     &            +4.d0/3.d0*dmu_dy(i,j,k)*dv_dy(i,1,k)
     &            -2.d0/3.d0*dmu_dy(i,j,k)*du_dx(i,1,k)
     &            -2.d0/3.d0*dmu_dy(i,j,k)*dw_dz(i,1,k)
     &            +dmu_dz(i,j,k)*dv_dz(i,1,k)
     &            +dmu_dz(i,j,k)*dw_dy(i,1,k))
            !MS$ENDIF 
     ...
     &          +d2w_dxz(i,j,k))      
     &        +1.d0/Re*(
     &          +4.d0/3.d0*dmu_dx(i,j,k)*du_dx(i,j,k)
     &          -2.d0/3.d0*dmu_dx(i,j,k)*dv_dy(i,j,k)
     &          -2.d0/3.d0*dmu_dx(i,j,k)*dw_dz(i,j,k)
     &          +dmu_dy(i,j,k)*du_dy(i,j,k)
     &          +dmu_dy(i,j,k)*dv_dx(i,j,k)
     &          +dmu_dz(i,j,k)*du_dz(i,j,k)
     &          +dmu_dz(i,j,k)*dw_dx(i,j,k)
     &        )
            E3_RK(i,j,k) =
     ...
     &          +d2w_dyz(i,j,k)) 
     &        +1.d0/Re*(
     &          +dmu_dx(i,j,k)*du_dy(i,j,k)
     &          +dmu_dx(i,j,k)*dv_dx(i,j,k)
     &          +4.d0/3.d0*dmu_dy(i,j,k)*dv_dy(i,j,k)
     &          -2.d0/3.d0*dmu_dy(i,j,k)*du_dx(i,j,k)
     &          -2.d0/3.d0*dmu_dy(i,j,k)*dw_dz(i,j,k)
     &          +dmu_dz(i,j,k)*dv_dz(i,j,k)
     &          +dmu_dz(i,j,k)*dw_dy(i,j,k)
     &        )
            E4_RK(i,j,k) = 
     ...
     &        +1.d0/Re*(
     &          +dmu_dx(i,j,k)*dw_dx(i,j,k)
     &          +dmu_dx(i,j,k)*du_dz(i,j,k)
     &          +dmu_dy(i,j,k)*dv_dz(i,j,k)
     &          +dmu_dy(i,j,k)*dw_dy(i,j,k)
     &          +4.d0/3.d0*dmu_dz(i,j,k)*dw_dz(i,j,k)
     &          -2.d0/3.d0*dmu_dz(i,j,k)*du_dx(i,j,k)
     &          -2.d0/3.d0*dmu_dz(i,j,k)*dv_dy(i,j,k)
     &        )
            !MS$IF (tp_proc.EQ.1)
              E5_RK(i,j,k) =
     &          -(Et(i,j,k)+p(i,j,k))*
     ...
     &          +w(i,j,k)*2.d0/3.d0*1.d0/Re*dmu_dz(i,j,k)*(
...
ccccc **********************************************************************
      SUBROUTINE der_x_D(local_imax,fc,dfc_dx)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2).OR.(tp_s.EQ.3)
        call der_x_cp_6th_P(fc,dfc_dx)
      !MS$ELSEIF (tp_s.EQ.4)        
        call der_x_cp_6th_D(fc,dfc_dx)
      !MS$ELSEIF (tp_s.EQ.5).OR.(tp_s.EQ.6)
        call der_x_ex_6th_D(fc,dfc_dx)
      !MS$ENDIF
...
ccccc           y direction. Dirichlet boundary condition was adopted.
ccccc **********************************************************************
      SUBROUTINE der_y_WN(local_imax,fc,dfc_dy)

      USE snmethod, only: der_y_cp_62th_D,dcy_dpy
      ...
      !MS$ELSEIF (tp_s.EQ.6)
        call der_y_cp_65th_WN(dcy_dpy,fc,dfc_dy)
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      !MS$ELSEIF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)
        call der_y_cp_62th_N(dcy_dpy,fc,dfc_dy)
      !MS$ELSEIF (tp_s.EQ.6)
        call der_y_cp_65th_NN(dcy_dpy,fc,dfc_dy)
      !MS$ENDIF  
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_xx_ex_6th_D

      integer local_imax
      real*8 fc(local_imax,jmax,kmax),d2fc_dx2(local_imax,jmax,kmax)

      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2).OR.(tp_s.EQ.3)
        call der_xx_cp_6th_P(fc,d2fc_dx2)
      !MS$ELSEIF (tp_s.EQ.4)
        call der_xx_cp_6th_D(fc,d2fc_dx2)
      !MS$ELSEIF (tp_s.EQ.5)
        call der_xx_ex_6th_D(fc,d2fc_dx2)
      !MS$ELSEIF (tp_s.EQ.6)  
        call der_xx_ex_6th_D(fc,d2fc_dx2)
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_yy_cp_62th_D,dcy_dpy,d2cy_dpy2
     ...
     &  d2fc_dy2(local_imax,jmax,kmax)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2)
        call der_yy_cp_6th_P(fc,d2fc_dy2)
      !MS$ELSEIF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)
        call der_yy_cp_62th_D(dcy_dpy,d2cy_dpy2,fc,dfc_dy,d2fc_dy2)
      !MS$ELSEIF (tp_s.EQ.6)
        call der_yy_cp_65th_WD(dcy_dpy,d2cy_dpy2,fc,dfc_dy,d2fc_dy2)
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_yy_cp_62th_N,dcy_dpy,d2cy_dpy2
     ...
     &  d2fc_dy2(local_imax,jmax,kmax)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2)
        call der_yy_cp_6th_P(fc,d2fc_dy2)
      !MS$ELSEIF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)
        call der_yy_cp_62th_N(dcy_dpy,d2cy_dpy2,fc,dfc_dy,d2fc_dy2)
      !MS$ELSEIF (tp_s.EQ.6)
        call der_yy_cp_65th_WD(dcy_dpy,d2cy_dpy2,fc,dfc_dy,d2fc_dy2)
      !MS$ENDIF
...
ccccc **********************************************************************
     ...
     &  d2fc_dy2(local_imax,jmax,kmax)
      ...
      !MS$ELSEIF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)
        call der_yy_cp_62th_N(dcy_dpy,d2cy_dpy2,fc,dfc_dy,d2fc_dy2)
      !MS$ELSEIF (tp_s.EQ.6)
        call der_yy_cp_65th_NN(dcy_dpy,d2cy_dpy2,fc,dfc_dy,d2fc_dy2)
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      call der_zz_cp_6th_P(fc,d2fc_dz2)
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_x_ex_6th_D,der_y_cp_62th_D,dcy_dpy,d2cy_dpy2
     ...
     &  d2fc_dxy(local_imax,jmax,kmax)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2)
        call der_x_cp_6th_P(fc,dfc_dx)
        call der_y_cp_6th_P(dfc_dx,d2fc_dxy)
      !MS$ELSEIF (tp_s.EQ.3)
        call der_x_cp_6th_P (fc,dfc_dx)
      ...
      !MS$ELSEIF (tp_s.EQ.6)  
        call der_x_ex_6th_D  (fc,dfc_dx)
        call der_y_cp_65th_WD(dcy_dpy,dfc_dx,d2fc_dxy)
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_x_ex_6th_D,der_z_cp_6th_P
     ...
     &  d2fc_dxz(local_imax,jmax,kmax)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2).OR.(tp_s.EQ.3)
        call der_x_cp_6th_P(fc,dfc_dx)
        call der_z_cp_6th_P(dfc_dx,d2fc_dxz)
      !MS$ELSEIF (tp_s.EQ.4)
        call der_x_cp_6th_D(fc,dfc_dx)
        call der_z_cp_6th_P(dfc_dx,d2fc_dxz)
      !MS$ELSEIF (tp_s.EQ.5)
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_z_cp_6th_P,der_y_cp_62th_D,dcy_dpy
     ...
     &  d2fc_dzy(local_imax,jmax,kmax)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2)
        call der_z_cp_6th_P(fc,dfc_dz)
        call der_y_cp_6th_P(dfc_dz,d2fc_dzy)
      !MS$ELSEIF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)
        call der_z_cp_6th_P (fc,dfc_dz)
        call der_y_cp_62th_D(dcy_dpy,dfc_dz,d2fc_dzy)
      !MS$ELSEIF (tp_s.EQ.6)
        call der_z_cp_6th_P  (fc,dfc_dz)
        call der_y_cp_65th_WD(dcy_dpy,dfc_dz,d2fc_dzy)
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_y_cp_62th_D,der_x_ex_6th_D,dcy_dpy,d2cy_dpy2
     ...
     &  d2fc_dyx(local_imax,jmax,kmax)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2)        
        call der_y_cp_6th_P(fc,dfc_dy)
        call der_x_cp_6th_P(dfc_dy,d2fc_dyx)
      !MS$ELSEIF (tp_s.EQ.3)
        call der_y_cp_62th_D(dcy_dpy,fc,dfc_dy)
        call der_x_cp_6th_P (dfc_dy,d2fc_dyx )
      !MS$ELSEIF (tp_s.EQ.4)
        call der_y_cp_62th_D(dcy_dpy,fc,dfc_dy)
        call der_x_cp_6th_D (dfc_dy,d2fc_dyx )
      !MS$ELSEIF (tp_s.EQ.5)
        call der_y_cp_62th_D(dcy_dpy,fc,dfc_dy)
        call der_x_ex_6th_D (dfc_dy,d2fc_dyx )
      !MS$ELSEIF (tp_s.EQ.6)  
        call der_y_cp_65th_WN(dcy_dpy,fc,dfc_dy)
        call der_x_ex_6th_D  (dfc_dy,d2fc_dyx )
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_y_cp_62th_N,der_x_ex_6th_D,dcy_dpy,d2cy_dpy2
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_z_cp_6th_P,der_x_ex_6th_D
     ...
     &  d2fc_dzx(local_imax,jmax,kmax)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2).OR.(tp_s.EQ.3)
        call der_z_cp_6th_P(fc,dfc_dz)
        call der_x_cp_6th_P(dfc_dz,d2fc_dzx)
      !MS$ELSEIF (tp_s.EQ.4)
        call der_z_cp_6th_P(fc,dfc_dz)
        call der_x_cp_6th_D(dfc_dz,d2fc_dzx)
      !MS$ELSEIF (tp_s.EQ.5)
        call der_z_cp_6th_P(fc,dfc_dz)
        call der_x_ex_6th_D(dfc_dz,d2fc_dzx)
      !MS$ELSEIF (tp_s.EQ.6)  
        call der_z_cp_6th_P(fc,dfc_dz)
        call der_x_ex_6th_D(dfc_dz,d2fc_dzx)
      !MS$ENDIF
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_y_cp_62th_D,der_z_cp_6th_P,dcy_dpy
     ...
     &  d2fc_dyz(local_imax,jmax,kmax)
      ...
      !MS$IF (tp_s.EQ.1).OR.(tp_s.EQ.2)
        call der_y_cp_6th_P(fc,dfc_dy)
        call der_z_cp_6th_P(dfc_dy,d2fc_dyz)
      !MS$ELSEIF (tp_s.EQ.3)
        call der_y_cp_62th_D(dcy_dpy,fc,dfc_dy)
        call der_z_cp_6th_P (dfc_dy,d2fc_dyz)
...
ccccc **********************************************************************
      ...
      USE snmethod, only: der_y_cp_62th_N,der_z_cp_6th_P,dcy_dpy
