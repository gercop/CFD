...
ccccc **********************************************************************
      ...
      !MS$ELSE
        ...
        if (my_rank.GE.pro) then
          local_sd(2) = my_rank-pro
        end if
        if (mod((my_rank+1),pro).NE.0) then
          local_sd(3) = my_rank+1
        end if    
        if (my_rank.LT.(pro-pro)) then
          local_sd(4) = my_rank+pro
        end if 
      !MS$ENDIF
...
ccccc ************************************************************************************************
     ...
     &  local_E1 (imax,jmax,kmax),local_E2 (imax,jmax,kmax),
     &  local_E3 (imax,jmax,kmax),local_E4 (imax,jmax,kmax),
     &  local_E5 (imax,jmax,kmax),global_E1(imax,jmax,kmax),
     &  global_E2(imax,jmax,kmax),global_E3(imax,jmax,kmax),
     &  global_E4(imax,jmax,kmax),global_E5(imax,jmax,kmax)   
     ...
     &      MPI_COMM_WORLD,ierror)
          ...
          SZ = (global_x2-global_x1+1)*jmax       

          tag = 3
          call MPI_SEND(local_E1(IB:IN,1:jmax,1),SZ,
     &      MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,ierror)
          tag = 4
          call MPI_SEND(local_E2(IB:IN,1:jmax,1),SZ,
     &      MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,ierror)
          tag = 5
          call MPI_SEND(local_E3(IB:IN,1:jmax,1),SZ,
     &      MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,ierror)
          tag = 6
          call MPI_SEND(local_E4(IB:IN,1:jmax,1),SZ,
     &      MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,ierror)
          tag = 7
          call MPI_SEND(local_E5(IB:IN,1:jmax,1),SZ,
     &      MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,ierror)
          tag = 8
     ...
     &        MPI_COMM_WORLD,status,ierror)  
            ...
            tag = 3
            call MPI_RECV(global_E1(IB:IN,1:jmax,1),SZ,
     &        MPI_DOUBLE_PRECISION,source,tag,
     &        MPI_COMM_WORLD,status,ierror)  
            tag = 4
            call MPI_RECV(global_E2(IB:IN,1:jmax,1),SZ,
     &        MPI_DOUBLE_PRECISION,source,tag,
     &        MPI_COMM_WORLD,status,ierror)  
            tag = 5
            call MPI_RECV(global_E3(IB:IN,1:jmax,1),SZ,
     &        MPI_DOUBLE_PRECISION,source,tag,
     &        MPI_COMM_WORLD,status,ierror)  
            tag = 6
            call MPI_RECV(global_E4(IB:IN,1:jmax,1),SZ,
     &        MPI_DOUBLE_PRECISION,source,tag,
     &        MPI_COMM_WORLD,status,ierror)  
            tag = 7
            call MPI_RECV(global_E5(IB:IN,1:jmax,1),SZ,
     &        MPI_DOUBLE_PRECISION,source,tag,
     &        MPI_COMM_WORLD,status,ierror)  
            tag = 8
