...
ccccc **********************************************************************
     ...
     &  E1_RK1,E2_RK1,E3_RK1,E4_RK1,E5_RK1)  
      ...
      call filter_x(local_imax,E1_t,E2_t,E3_t,E4_t,E5_t)
...
ccccc **************************************************************************
     ...
     &  E1_RK,E2_RK,E3_RK,E4_RK,E5_RK)
      ...
      USE snmethod,only: stx_ex_6th_D
     ...
     &  du_0_dy  (local_imax,jmax,kmax),d2u_0_dy2(local_imax,jmax,kmax)
      ...
      if (pro.NE.1) then
        call exchange_ghostpoints(jmax,kmax,E1,E2,E3,E4,E5)
      end if
...
ccccc ----------------------------------      
      ...
      call der_x_D(my_rank,pro,local_imax,stx_ex_6th_D,mu, dmu_dx )
...
ccccc ---------------------------------
     ...
     &            +d2w_dyz(i,1,k)) 
     &          +1.d0/Re*(
     &            +dmu_dx(i,1,k)*du_dy(i,1,k)
     &            +dmu_dx(i,1,k)*dv_dx(i,1,k)
     &            +4.d0/3.d0*dmu_dy(i,1,k)*dv_dy(i,1,k)
     &            -2.d0/3.d0*dmu_dy(i,1,k)*du_dx(i,1,k)
     &            -2.d0/3.d0*dmu_dy(i,1,k)*dw_dz(i,1,k)
     &            +dmu_dz(i,1,k)*dv_dz(i,1,k)
     &            +dmu_dz(i,1,k)*dw_dy(i,1,k))
            !MS$ENDIF 
...
ccccc **********************************************************************
     ...
     &  dfc_dx(local_imax,jmax,kmax)
      ...
      !MS$ELSEIF (tp_s.EQ.5).OR.(tp_s.EQ.6)
        call der_x_ex_6th_D(local_imax,jmax,kmax,stx,fc,dfc_dx)
      !MS$ENDIF
...
ccccc **********************************************************************
     ...
     &  der_y_cp_65th_WD,dcy_dpy,stx_ex_6th_D
     ...
     &  d2fc_dxy(local_imax,jmax,kmax)
      ...
      !MS$ELSEIF (tp_s.EQ.4)
        ...
        call der_y_cp_62th_D(dcy_dpy,dfc_dx,d2fc_dxy)
      !MS$ELSEIF (tp_s.EQ.5)
        call der_x_ex_6th_D(local_imax,jmax,kmax,stx_ex_6th_D,fc,dfc_dx)
        !MS$IF (stretching_in_y.EQ.0)     
      ...
      !MS$ELSEIF (tp_s.EQ.6)  
        call der_x_ex_6th_D(local_imax,jmax,kmax,stx_ex_6th_D,fc,dfc_dx)
        !MS$IF (stretching_in_y.EQ.0)
          call der_y_cp_65th_WD(dy,local_imax,jmax,kmax,dcy_dpy,dfc_dx,d2fc_dxy)
        !MS$ELSEIF (stretching_in_y.EQ.1) 
          call der_y_cp_65th_WD(dcy,local_imax,jmax,kmax,dcy_dpy,dfc_dx,d2fc_dxy)
        !MS$ENDIF
      !MS$ENDIF
...
ccccc **********************************************************************
     ...
     &      dcy_dpy,fc,dfc_dy)
        !MS$ENDIF 
        call der_x_ex_6th_D(local_imax,jmax,kmax,stx_ex_6th_D,dfc_dy,d2fc_dyx)
      !MS$ELSEIF (tp_s.EQ.6)  
        !MS$IF (stretching_in_y.EQ.0)     
          call der_y_cp_65th_WN(dy,local_imax,jmax,kmax,dcy_dpy,fc,dfc_dy)
        !MS$ELSEIF (stretching_in_y.EQ.1) 
...
ccccc **********************************************************************
     ...
     &  d2fc_dzx(local_imax,jmax,kmax)
      ...
      !MS$ELSEIF (tp_s.EQ.5).OR.(tp_s.EQ.6)
        call der_z_cp_6th_P(dz,local_imax,jmax,kmax,fc,dfc_dz)
        call der_x_ex_6th_D(local_imax,jmax,kmax,stx_ex_6th_D,dfc_dz,d2fc_dzx)
      !MS$ENDIF
