...
ccccc **********************************************************************
      ...
      write (*,*) my_rank,'local_sd   :',local_sd(1)
      write (*,*) my_rank,'local_sd   :',local_sd(3)
...
ccccc **********************************************************************
      SUBROUTINE exchange_ghostpoints(jmax,kmax,E1,E2,E3,E4,E5)

      integer jmax,kmax,tag,IB,IN,SZ
      real*8 
     ...
     &  E5(local_imax,jmax,kmax)
      ...
      !MS$IF (parallel.EQ.1)
        if (mod((my_rank+1),pro).NE.0) then
          ...
          SZ  = jmax*kmax*ghost_points  

          tag = 1       
          call MPI_SEND(E1(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,ierror)
          tag = 3       
          call MPI_SEND(E2(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,ierror)
          tag = 5       
          call MPI_SEND(E3(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,ierror)
          tag = 7       
          call MPI_SEND(E4(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,ierror)
          tag = 9       
          call MPI_SEND(E5(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,ierror)
        end if
        ...
        if (mod((my_rank),pro).NE.0) then
          ...
          tag = 2
          call MPI_SEND(E1(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(1),tag,
     &      MPI_COMM_WORLD,ierror)
          tag = 4
          call MPI_SEND(E2(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(1),tag,
     ...
     &      MPI_COMM_WORLD,ierror)
        ...
        if (mod((my_rank+1),pro).NE.0) then
          ...
          tag = 2
          call MPI_RECV(E1(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,status,ierror)  
          tag = 4
          call MPI_RECV(E2(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,status,ierror)  
          tag = 6
          call MPI_RECV(E3(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,status,ierror)  
          tag = 8
          call MPI_RECV(E4(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,status,ierror)  
          tag = 10
          call MPI_RECV(E5(IB:IN,1:jmax,1:kmax),SZ,
     &      MPI_DOUBLE_PRECISION,local_sd(3),tag,
     &      MPI_COMM_WORLD,status,ierror)  
        end if
