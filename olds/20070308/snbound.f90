...
cc      write(*,*) stx(1,1)*120.d0*0.5d0
cc      write(*,*) stx(1,2)*120.d0*0.5d0
cc      write(*,*) stx(1,3)*120.d0*0.5d0
cc      write(*,*) stx(1,4)*120.d0*0.5d0
cc      write(*,*) stx(1,5)*120.d0*0.5d0
cc      write(*,*) stx(1,6)*120.d0*0.5d0
cc      stop
...
cc      write(*,*) stxx(local_imax,1)*360.d0*0.5d0**2.d0
cc      write(*,*) stxx(local_imax,2)*360.d0*0.5d0**2.d0
cc      write(*,*) stxx(local_imax,3)*360.d0*0.5d0**2.d0
cc      write(*,*) stxx(local_imax,4)*360.d0*0.5d0**2.d0
cc      write(*,*) stxx(local_imax,5)*360.d0*0.5d0**2.d0
cc      write(*,*) stxx(local_imax,6)*360.d0*0.5d0**2.d0
cc      write(*,*) stxx(local_imax,7)*360.d0*0.5d0**2.d0
cc      stop
...
ccccc **********************************************************************
     ...
     &  stx,stxx,rho,u,v,w,Et,p,tp,mu)
      ...
      real*8 t,yy,zz,u_1,v_1,w_1,A_1,A_2,phi,C2,x(local_imax),
      ...
      !MS$IF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)
        do k=1,kmax
          do i=1,local_imax
            v  (i,1,k)    = 0.d0
            v  (i,jmax,k) = 0.d0            
cc            rho(i,1,k)    = p(i,1,k)/((gamma-1.d0)*e(i,1,k))       !Atualizado 08/03/2007
cc            rho(i,jmax,k) = p(i,jmax,k)/((gamma-1.d0)*e(i,jmax,k)) !Atualizado 08/03/2007
          end do
        end do
      !MS$ENDIF       
