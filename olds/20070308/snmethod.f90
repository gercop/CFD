      ...
      save
        ...
        real*8,  dimension(:,:), allocatable :: global_stxx_ex_6th_D
...
ccccc **********************************************************************
      SUBROUTINE der_x_ex_6th_D(my_rank,pro,imax,jmax,kmax,stx,fc,dfc_dx)
...
cc            write(*,*) stx(3,1)*120.d0*0.5d0
cc            write(*,*) stx(3,2)*120.d0*0.5d0
cc            write(*,*) stx(3,3)*120.d0*0.5d0
cc            write(*,*) stx(3,4)*120.d0*0.5d0
cc            write(*,*) stx(3,5)*120.d0*0.5d0
cc            write(*,*) stx(3,6)*120.d0*0.5d0
cc            stop
          end if  
...
cc            write(*,*) stx(i,1)*720.d0*0.5d0
cc            write(*,*) stx(i,2)*720.d0*0.5d0
cc            write(*,*) stx(i,3)*720.d0*0.5d0
cc            write(*,*) stx(i,4)*720.d0*0.5d0
cc            write(*,*) stx(i,5)*720.d0*0.5d0
cc            write(*,*) stx(i,6)*720.d0*0.5d0
cc            write(*,*) stx(i,7)*720.d0*0.5d0
cc            write(*,*) 
          end do
          ...
          if (my_rank+1.EQ.pro) then 
            do i=imax-2,imax 
              dfc_dx(i,j,k) = 
     &          stx(i,1)*fc(imax-5,j,k)+
     &          stx(i,2)*fc(imax-4,j,k)+
     &          stx(i,3)*fc(imax-3,j,k)+
     &          stx(i,4)*fc(imax-2,j,k)+
     &          stx(i,5)*fc(imax-1,j,k)+
     &          stx(i,6)*fc(imax  ,j,k)
cc              write(*,*) stx(imax,1)*120.d0*0.5d0
cc              write(*,*) stx(imax,2)*120.d0*0.5d0
cc              write(*,*) stx(imax,3)*120.d0*0.5d0
cc              write(*,*) stx(imax,4)*120.d0*0.5d0
cc              write(*,*) stx(imax,5)*120.d0*0.5d0
cc              write(*,*) stx(imax,6)*120.d0*0.5d0
cc              write(*,*) 
cc              stop
            end do  
          end if  
...
ccccc **********************************************************************
      ...
      real*8 fc(imax,jmax,kmax),d2fc_dx2(imax,jmax,kmax),stxx(imax,7)
...
cc              write(*,*) stxx(3,1)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(3,2)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(3,3)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(3,4)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(3,5)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(3,6)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(3,7)*360.d0*0.5d0**2.d0
cc              stop
            end do
          end if
...
cc            write(*,*) stxx(i,1)*360.d0*0.5d0**2.d0
cc            write(*,*) stxx(i,2)*360.d0*0.5d0**2.d0
cc            write(*,*) stxx(i,3)*360.d0*0.5d0**2.d0
cc            write(*,*) stxx(i,4)*360.d0*0.5d0**2.d0
cc            write(*,*) stxx(i,5)*360.d0*0.5d0**2.d0
cc            write(*,*) stxx(i,6)*360.d0*0.5d0**2.d0
cc            write(*,*) stxx(i,7)*360.d0*0.5d0**2.d0
cc            write(*,*)
          end do
cc          stop
...
cc              write(*,*) stxx(imax,1)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(imax,2)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(imax,3)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(imax,4)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(imax,5)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(imax,6)*360.d0*0.5d0**2.d0
cc              write(*,*) stxx(imax,7)*360.d0*0.5d0**2.d0
cc              stop
            end do  
          end if
        end do
...
ccccc                  in the y-direction using the 6th order centered 
ccccc                  compact finite-difference scheme. Periodic boundary 
ccccc                  condition was adopted.
ccccc **********************************************************************
...
ccccc                   in the y-direction using the 2nd and 6th order 
ccccc                   compact finite-difference schemes. Dirichlet 
ccccc                   boundary condition was used.
ccccc **********************************************************************
...
ccccc                    condition was used to simulated the wall.
ccccc **********************************************************************
      SUBROUTINE der_yy_cp_65th_WD(dy,imax,jmax,kmax,
     &  mtrc_d1,mtrc_d2,fc,dfc_dy,d2fc_dy2)      
...
ccccc                    boundary condition was used to simulated the wall.
ccccc **********************************************************************
      SUBROUTINE der_yy_cp_65th_WN(dy,imax,jmax,kmax,
     &  mtrc_d1,mtrc_d2,fc,dfc_dy,d2fc_dy2)      
...
ccccc                    condition was adopted.
ccccc **********************************************************************
      SUBROUTINE der_yy_cp_65th_NN(dy,imax,jmax,kmax,
     &  mtrc_d1,mtrc_d2,fc,dfc_dy,d2fc_dy2)      
...
ccccc                system of second derivatives in the y-direction. The  
ccccc                2nd and 6th order compact finite-difference schemes 
ccccc                were used. Dirichlet boundary condition was adopted.
ccccc **********************************************************************
...
ccccc                system of second derivatives in the y-direction. The  
ccccc                2nd and 6th order compact finite-difference schemes   
ccccc                were used. Neumann boundary condition was adopted.
ccccc **********************************************************************
     ...
     &    +02.d0*mtrc_d2(j+1)/(mtrc_d1(j+1)**3.d0)*dfc_dy(i,j+1,k) 
      ...
      r(jmax-1)=(fc(i,jmax-2,k)-2.d0*fc(i,jmax-1,k)+fc(i,jmax,k))/(dy*dy)
     &  +mtrc_d2(jmax-1)/(mtrc_d1(jmax-1)**3.d0)*dfc_dy(i,jmax-1,k)
      r(jmax)=(-fc(i,jmax-2,k)+8.d0*fc(i,jmax-1,k)-7.d0*fc(i,jmax,k))
     &  /(2.d0*dy*dy)
     &  +mtrc_d2(jmax)/(mtrc_d1(jmax)**3.d0)*dfc_dy(i,jmax,k)
...
ccccc                system of second derivatives. The 2nd and 6th order 
ccccc                compact finite-difference schemes were used for 
...
ccccc **********************************************************************
      ...
      aa(1)=0.d0
      bb(1)=1.d0/(mtrc_d1(1)**2.d0)
      cc(1)=0.d0

      aa(2)=0.d0
      bb(2)=1.d0/(mtrc_d1(2)**2.d0)
      cc(2)=0.d0
      ...
      aa(lmax-1)=0.d0
      bb(lmax-1)=1.d0/(mtrc_d1(lmax-1)**2.d0)
      cc(lmax-1)=0.d0

      aa(lmax)=0.d0
      bb(lmax)=1.d0/(mtrc_d1(lmax)**2.d0)
      cc(lmax)=0.d0
...
ccccc **********************************************************************
      ...
      if (my_rank+1.EQ.pro) then
        do i=imax-2,imax
          call coeffs_d1_ex_5th(x(imax-5),x(imax-4),x(imax-3),x(imax-2),x(imax-1),x(imax),x(i),1,stx(i,1))
          call coeffs_d1_ex_5th(x(imax-5),x(imax-4),x(imax-3),x(imax-2),x(imax-1),x(imax),x(i),2,stx(i,2))
          call coeffs_d1_ex_5th(x(imax-5),x(imax-4),x(imax-3),x(imax-2),x(imax-1),x(imax),x(i),3,stx(i,3))
          call coeffs_d1_ex_5th(x(imax-5),x(imax-4),x(imax-3),x(imax-2),x(imax-1),x(imax),x(i),4,stx(i,4))
          call coeffs_d1_ex_5th(x(imax-5),x(imax-4),x(imax-3),x(imax-2),x(imax-1),x(imax),x(i),5,stx(i,5))
          call coeffs_d1_ex_5th(x(imax-5),x(imax-4),x(imax-3),x(imax-2),x(imax-1),x(imax),x(i),6,stx(i,6))
          stx(i,7) = 0.d0
        end do
      end if
