...
ccccc **********************************************************************
      ...
      if ((jmax.EQ.1).AND.(kmax.EQ.1)) then 
        ...
        call read_BIN_3D(fname_aux,U1,U2,U3,U4,U5,error)
      end if
...
ccccc **********************************************************************
      ...
      integer i,j,k,local_imax
      real*8 
     &  x(local_imax),y(jmax),z(kmax),xx,yy,zz,u_1,v_1,w_1,
     ...
     &          (u_0(1,j,k)**2.d0+M2/M1-u_0(1,j,k)*(1.d0+M2/M1))
	      ...
	      u_1 = 
     &          -02.d0*A_1*sigma*yy*dexp(-sigma*yy**2.d0)/alpha*dsin(alpha*xx/1.d0)
     &          -04.d0*A_2*sigma*yy*dexp(-sigma*yy**2.d0)/alpha*dsin(alpha*xx/2.d0)
     ...
     &          -16.d0*A_4*sigma*yy*dexp(-sigma*yy**2.d0)/alpha*dsin(alpha*xx/8.d0)
              v_1 = 
     &          +A_1*dcos(alpha*xx/1.d0)*dexp(-sigma*yy**2.d0)
     &          +A_2*dcos(alpha*xx/2.d0)*dexp(-sigma*yy**2.d0)
     ...
     &          +A_4*dcos(alpha*xx/2.d0+beta*zz)+A_4*dcos(alpha*xx/2.d0-beta*zz))*dexp(-sigma*yy**2.d0)
              w_1 = 
     &          (A_1*dcos(alpha*xx/1.d0+phi)
     &          +A_2*dcos(alpha*xx/2.d0+phi)
     &          +A_3*dcos(alpha*xx/1.d0+beta*zz)+A_3*dcos(alpha*xx/1.d0-beta*zz)
     &          +A_4*dcos(alpha*xx/2.d0+beta*zz)+A_4*dcos(alpha*xx/2.d0-beta*zz))*dexp(-sigma*yy**2.d0)
            ...
            !MS$IF (tp_s.EQ.4)      !Velocity profile for Temporal development
              u  (i,j,k) = u_0(1,j,k) + 1.d0/(Ma*c)*u_1  
              v  (i,j,k) = v_0(1,j,k) + 1.d0/(Ma*c)*v_1  
