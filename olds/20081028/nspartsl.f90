...
cccccc Date      : 02/05/2007                                             *
cccccc Update    : 28/05/2007                                             *
cccccc Version	 : 3.6                                                    *
cccccc File      : snpar.f90                                              *
cccccc ********************************************************************
      character*7 dirgraf

      integer
     &  qtimes,imax,imax_phy,jmax,kmax,tmin,tmax

      real*8
     &  Pi,c,A_1,A_2,A_3,A_4,alpha_1,alpha_2,kapa,beta,ffreq,omega,x_0,y_0,z_0,
     &  Lx,Ly,Lz,Lx_spo,dx,dy,dz,dcx,dcy,dcz,dt,
     &  CFL,D,sigma,gamma,Ma,Re,Pr,M1,M2

      !MS$DEFINE parallel        = 00

      !MS$DEFINE tp_s            = 04    !Simulation type:
                                         !  1 = Model Equation;
                                         !  2 = Progressive Wave;
                                         !  3 = Standing Wave;
      ...
      !MS$DEFINE tp_proc         = 02    !Process type:
                                         !  1 = Full energy equation flow; 
                                         !  2 = Isentropic flow (Adiabatic);

      !MS$DEFINE tp_viscous      = 02    !Viscous type:
                                         !  1 = Suntherland law; 
                                         !  2 = Dynamic Viscosity constant;

      !MS$DEFINE cancel_alongvis = 00    !Cancel alongment viscous in y-direction

      !MS$DEFINE filter_in_x     = 01    !Compact filter in x-direction
      !MS$DEFINE filter_in_y     = 01    !Compact filter in y-direction
      !MS$DEFINE filter_in_z     = 00    !Compact filter in z-direction

      !MS$DEFINE stretching_in_x = 00	 !Stretching in x-direction
      !MS$DEFINE stretching_in_y = 01    !Stretching in y-direction
      !MS$DEFINE stretching_in_z = 00    !Stretching in z-direction
     ...
     &  imax          = 00000000041, !Maximum size in x-direction 
     &  jmax          = 00000000081, !Maximum size in y-direction 
     &  kmax          = 00000000001, !Maximum size in z-direction 
     &  tmin          = 00000000000, !Maximum size in time
     &  tmax          = 01000000000, !Maximum size in time

     &  imax_phy      = imax-000100) !Physical domain - Points from 0..imax_phy

      parameter (
     &  Pi       = 3.141592653589793d0,       !Pi

     &  Ma       = 0.4d0,                     !Mach Number, dimensionless 
...
ccccc ***********************************
     ...
     &  M2       = -Ma,                       !Mach number for y<0, dimensionless 

     &  A_1      = 0.00010000d0,              !Wave amplitude (Fluctuation) - m
     &  A_2      = A_1/10.d0,                 !Wave amplitude (Fluctuation) - m
     &  A_3      = 0.d0,                      !Wave amplitude (Fluctuation) - m
     &  A_4      = 0.d0,                      !Wave amplitude (Fluctuation) - m
     &  x_0      = 0.d0,                      !Initial domain in x-direction - m
     &  y_0      = 0.d0,                      !Initial domain in y-direction - m
     &  z_0      = 0.d0,                      !Initial domain in z-direction - m
     &  Lx       = 016.000000d0-x_0,          !Comp. domain in x-direction - m
     &  Ly       = 024.000000d0-y_0,          !Comp. domain in y-direction - m
     &  Lz       = 050.000000d0-z_0,          !Comp. domain in z-direction - m
     &  Lx_spo   = 600.d0,                    !Sponge domain extend from Lx to Lx_spo - m
     &  alpha_1  = 4.d0*Pi/Lx,                !Wavenumber in x-direction - Fundamental - 1/m
     &  alpha_2  = 2.d0*Pi/Lx,                !Wavenumber in x-direction - Subharmonic - 1/m
     &  kapa     = 2.d0*Pi/Ly,                !Wavenumber in y-direction - 1/m
     &  beta     = 2.d0*Pi/Lz,                !Wavenumber in z-direction - 1/m
