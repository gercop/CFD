...
ccccc **********************************************************************
      SUBROUTINE der_x_ex_6th_D(dx,my_rank,pro,imax,jmax,kmax,stx,fc,dfc_dx)

      IMPLICIT NONE

      integer i,j,k,imax,jmax,kmax,my_rank,pro
      real*8 dx,fc(imax,jmax,kmax),dfc_dx(imax,jmax,kmax),stx(imax,7)
...
cc            end do
            ...
            dfc_dx(1,j,k) = 1.d0/(120.d0*dx)*(-274.d0*fc(1,j,k)
     &        +600.d0*fc(2,j,k)-600.d0*fc(3,j,k)+400.d0*fc(4,j,k)
     &        -150.d0*fc(5,j,k)+24.d0*fc(6,j,k))
            dfc_dx(2,j,k) = 1.d0/(120.d0*dx)*(-24.d0*fc(1,j,k)
     &        -130.d0*fc(2,j,k)+240.d0*fc(3,j,k)-120.d0*fc(4,j,k)
     &        +40.d0*fc(5,j,k)-6.d0*fc(6,j,k))
            dfc_dx(3,j,k) = 1.d0/(120.d0*dx)*(6.d0*fc(1,j,k)
     &        -60.d0*fc(2,j,k)-40.d0*fc(3,j,k)+120.d0*fc(4,j,k)
     ...
     &          stx(i,7)*fc(imax,j,k)
            ...
            dfc_dx(imax-2,j,k) = 1.d0/(120.d0*dx)*(-6.d0*fc(imax,j,k)
     &        +60.d0*fc(imax-1,j,k)+40.d0*fc(imax-2,j,k)-120.d0*fc(imax-3,j,k)
     &        +30.d0*fc(imax-4,j,k)-4.d0*fc(imax-5,j,k))

            dfc_dx(imax-1,j,k) = 1.d0/(120.d0*dx)*(24.d0*fc(imax,j,k)
     &        +130.d0*fc(imax-1,j,k)-240.d0*fc(imax-2,j,k)+120.d0*fc(imax-3,j,k)
     &        -40.d0*fc(imax-4,j,k)+6.d0*fc(imax-5,j,k))

            dfc_dx(imax,j,k) = 1.d0/(120.d0*dx)*(274.d0*fc(imax,j,k)
     &        -600.d0*fc(imax-1,j,k)+600.d0*fc(imax-2,j,k)-400.d0*fc(imax-3,j,k)
     &        +150.d0*fc(imax-4,j,k)-24.d0*fc(imax-5,j,k))
          end if
        end do
      end do
...
ccccc                  adopted.
ccccc **********************************************************************
      SUBROUTINE der_xx_ex_6th_D(dx,imax,jmax,kmax,stxx,fc,d2fc_dx2)
      ...
      real*8 dx,fc(imax,jmax,kmax),d2fc_dx2(imax,jmax,kmax),stxx(imax,7)
...
cc            do i=1,3 
cc              d2fc_dx2(i,j,k) = 
cc     &          stxx(i,1)*fc(1,j,k)+
cc     &          stxx(i,2)*fc(2,j,k)+
cc     &          stxx(i,3)*fc(3,j,k)+
cc     &          stxx(i,4)*fc(4,j,k)+
cc     &          stxx(i,5)*fc(5,j,k)+
cc     &          stxx(i,6)*fc(6,j,k)+
cc     &          stxx(i,7)*fc(7,j,k)
cc            end do

            d2fc_dx2(1,j,k) = 1.d0/(60.d0*dx*dx)*(225.d0*fc(1,j,k)
     &        -770.d0*fc(2,j,k)+1070.d0*fc(3,j,k)-780.d0*fc(4,j,k)
     &        +305.d0*fc(5,j,k)-50.d0*fc(6,j,k))
            d2fc_dx2(2,j,k) = 1.d0/(60.d0*dx*dx)*(50.d0*fc(1,j,k)
     &        -75.d0*fc(2,j,k)-20.d0*fc(3,j,k)+70.d0*fc(4,j,k)
     &        -30.d0*fc(5,j,k)+5.d0*fc(6,j,k))
            d2fc_dx2(3,j,k) = 1.d0/(60.d0*dx*dx)*(-5.d0*fc(1,j,k)
     &        +80.d0*fc(2,j,k)-150.d0*fc(3,j,k)+80.d0*fc(4,j,k)
     &        -5.d0*fc(5,j,k)+0.d0*fc(6,j,k))
          end if
     ...
     &        stxx(i,7)*fc(i+3,j,k)
          ...
          if (my_rank+1.EQ.pro) then 
cc            do i=imax-2,imax-1
cc              d2fc_dx2(i,j,k) =
cc     &          stxx(i,1)*fc(imax-6,j,k)+
cc     &          stxx(i,2)*fc(imax-5,j,k)+
cc     &          stxx(i,3)*fc(imax-4,j,k)+
cc     &          stxx(i,4)*fc(imax-3,j,k)+
cc     &          stxx(i,5)*fc(imax-2,j,k)+
cc     &          stxx(i,6)*fc(imax-1,j,k)+
cc     &          stxx(i,7)*fc(imax-0,j,k)
cc            end do

            d2fc_dx2(imax-2,j,k) = 1.d0/(60.d0*dx*dx)*(-5.d0*fc(imax,j,k)
     &        +80.d0*fc(imax-1,j,k)-150.d0*fc(imax-2,j,k)+80.d0*fc(imax-3,j,k)
     &        -5.d0*fc(imax-4,j,k)+0.d0*fc(imax-5,j,k))
            d2fc_dx2(imax-1,j,k) = 1.d0/(60.d0*dx*dx)*(50.d0*fc(imax,j,k)
     &        -75.d0*fc(imax-1,j,k)-20.d0*fc(imax-2,j,k)+70.d0*fc(imax-3,j,k)
     &        -30.d0*fc(imax-4,j,k)+5.d0*fc(imax-5,j,k))
            d2fc_dx2(imax,j,k) = 1.d0/(60.d0*dx*dx)*(225.d0*fc(imax,j,k)
     &        -770.d0*fc(imax-1,j,k)+1070.d0*fc(imax-2,j,k)-780.d0*fc(imax-3,j,k)
     &        +305.d0*fc(imax-4,j,k)-50.d0*fc(imax-5,j,k))
          end if
        end do
      end do
