...
ccccc **********************************************************************
     ...
     &  du_dy(imax,jmax,kmax),dv_dx(imax,jmax,kmax)
      ...
      write(*,*) global_stx_ex_6th_D
      read(*,*)
      call der_x_D (0,1,imax,global_stx_ex_6th_D,v,dv_dx)
cc      call der_x_D (0,1,imax,global_stx_ex_6th_D,w,dw_dx)
...
ccccc **********************************************************************
      SUBROUTINE sv_TCP_ALL_PAR_SASC_1D(fname,x,y,z,t)

      USE snmethod,  only: global_stx_ex_6th_D,global_stxx_ex_6th_D 
     ...
     &  global_E4,global_E5,rho,u,v,w,Et,p,tp,mu)
      call boundflow(0,1,imax,x,y,z,t,global_stx_ex_6th_D,
     &  global_stxx_ex_6th_D,rho,u,v,w,Et,p,tp,mu)
...
ccccc **********************************************************************
      SUBROUTINE sv_TCP_ALL_PAR_SASC_2D(fname,x,y,z,t)
...
ccccc **********************************************************************
      SUBROUTINE sv_TCP_ALL_PAR_SASC_3D(fname,x,y,z,t)
...
ccccc **********************************************************************
      SUBROUTINE sv_TCP_ALL_SN_SASC(enbd,fname,x,y,z,tt)
      ...
      if ((jmax.EQ.1).AND.(kmax.EQ.1)) then
        call sv_TCP_ALL_PAR_SASC_1D(fname_aux,x,y,z,dble(tt)*dt)
      else if ((jmax.NE.1).AND.(kmax.EQ.1)) then
        call sv_TCP_ALL_PAR_SASC_2D(fname_aux,x,y,z,dble(tt)*dt)
      else if ((jmax.NE.1).AND.(kmax.NE.1)) then
        call sv_TCP_ALL_PAR_SASC_3D(fname_aux,x,y,z,dble(tt)*dt)
      end if
...
ccccc **********************************************************************
      SUBROUTINE sv_TCP_ALL_SN_SBIN(enbd,fname,x,y,z,tt)
      ...
      if ((jmax.EQ.1).AND.(kmax.EQ.1)) then
        call sv_TCP_ALL_PAR_SASC_1D(fname_aux,x)
      else if ((jmax.NE.1).AND.(kmax.EQ.1)) then
        call write_BIN_2D(fname_aux,x,y,z)
      else if ((jmax.NE.1).AND.(kmax.NE.1)) then
...
ccccc **********************************************************************
     ...
     &  global_stx_ex_6th_D,global_stxx_ex_6th_D
      USE sninitial, only: global_x,global_E1,global_E2,global_E3,
     &  global_E4,global_E5
     ...
     &  global_E1,global_E2,global_E3,global_E4,global_E5)
      ...
      if ((tt.EQ.0).OR.(tt.EQ.tmin+1)) then
        ...
        open (1,file=dirgraf//'x-grid.dat',access='sequential')
        write(1,*) 'VARIABLES= n,df'
        write(1,*) 'ZONE I=',imax,',F=POINT'
        do i=1,imax-1
          write(1,*) DBLE(i),abs(global_x(i+1)-global_x(i))
        end do
        write(1,*) DBLE(imax),abs(global_x(imax)-global_x(imax-1))
        close (unit=1)          
        open (1,file=dirgraf//'y-grid.dat',access='sequential')
        write(1,*) 'VARIABLES= n,df'
        write(1,*) 'ZONE I=',jmax,',F=POINT'
        do j=1,jmax
          write(1,*) DBLE(j),abs(y(j+1)-y(j))
        end do
        write(1,*) DBLE(jmax),abs(y(jmax)-y(jmax-1))
        close (unit=1)
      end if
...
ccccc *****************************************
      !MS$IF (sv_TEC_BIN.EQ.1)
        call sv_TCP_ALL_SN_SBIN(sv_TCP_SNS,'SN__S',global_x,y,z,tt) 
      !MS$ENDIF
      !MS$IF (sv_TEC_ASC.EQ.1)
        call sv_TCP_ALL_SN_SASC(sv_TCP_SNS,'SN__S',global_x,y,z,tt)
      !MS$ENDIF
