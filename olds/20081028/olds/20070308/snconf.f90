...
ccccc ----------------------------------      
      ...
      !MS$IF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)                  
        ...
        call der_y_WN(local_imax,rho,drho_dy) !Atualizado 08/03/2007
...
ccccc ---------------------------------
      ...
      do k=1,kmax 
        do j=1,jmax 
          do i=1,local_imax
            dmu_dtp(i,j,k) = (3.d0/2.d0)*dsqrt(tp(i,j,k))*(1.d0+C2)/(tp(i,j,k)+C2)
     &        -tp(i,j,k)**(3.d0/2.d0)*(1.d0+C2)/((tp(i,j,k)+C2)**2.d0)
            !MS$IF (tp_s.EQ.6)
              dp_dy(i,1,k) = 
