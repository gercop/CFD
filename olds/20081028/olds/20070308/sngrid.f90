...
cccccc ************************************
      ...
      !MS$ELSEIF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)
        do j=1,jmax 
          y(j)           = dble(j-1)*dy 
          du_0_dy_aux(j) = c*(M1-M2)*(1.d0-tanh(2.d0*y(j))**2.d0)  
        end do
...
ccccc **********************************************************************
      ...
      USE snmpi,only: my_rank,pro,local_imax,global_x1,global_x2,ghost_points

      IMPLICIT NONE
      INCLUDE 'snparv36.f90'

      integer i,j,k
      real*8 x(imax),y(jmax),z(kmax),local_x1,local_x2,
