      ...
      save
        ...
        real*8,  dimension(:,:,:), allocatable :: E1
        real*8,  dimension(:,:,:), allocatable :: E2
        real*8,  dimension(:,:,:), allocatable :: E3
        real*8,  dimension(:,:,:), allocatable :: E4
        real*8,  dimension(:,:,:), allocatable :: E5

        real*8,  dimension(imax)               :: global_x
        real*8,  dimension(imax,jmax,kmax)     :: global_E1
        real*8,  dimension(imax,jmax,kmax)     :: global_E2
        real*8,  dimension(imax,jmax,kmax)     :: global_E3
        real*8,  dimension(imax,jmax,kmax)     :: global_E4
        real*8,  dimension(imax,jmax,kmax)     :: global_E5
...
ccccc **********************************************************************
      ...
      !MS$IF (tp_s.EQ.3).OR.(tp_s.EQ.4).OR.(tp_s.EQ.5)         
        call ini_cond_slayer(local_imax,x,y,z)
      !MS$ELSEIF (tp_s.EQ.6)                
        call ini_cond_blayer(local_imax,x,y,z)
      !MS$ENDIF
       
      if (tmin.NE.0) then
        call ini_cond_file(local_imax,x,y,z,tmin)
      end if
...
ccccc **********************************************************************
      SUBROUTINE ini_cond_file(local_imax,x,y,z,tt)
      
      character*05 fname
      character*26 fname_aux
      integer i,j,k,local_imax,error,tt
      real*8 x(local_imax),y(jmax),z(kmax),
     &  wx (local_imax,jmax,kmax),wy (local_imax,jmax,kmax),
     &  wz (local_imax,jmax,kmax),Qi (local_imax,jmax,kmax)
      ...
      if ((jmax.EQ.1).AND.(kmax.EQ.1)) then 
        ...
        call read_BIN_3D(fname_aux,x,y,z,u,v,w,wx,wy,wz,Qi,rho,p,tp,Et,error)
      end if
...
ccccc **********************************************************************
      SUBROUTINE ini_cond_slayer(local_imax,x,y,z)
     ...
     &              dexp(-sigma*yy**2.d0)
            ...
            E2(i,j,k) = rho_0(i,j,k)*u(i,j,k)
            E3(i,j,k) = rho_0(i,j,k)*v(i,j,k)          
            E4(i,j,k) = rho_0(i,j,k)*w(i,j,k)          
...
ccccc **********************************************************************
      ...
      parameter (imach=2) !Size of the vector in program profcom
      integer ioerr,nt,nz,ny,nx,nparam,ntimes(512),ninf
      real*8 u_ini(nmax),v_ini(nmax),rho_ini(nmax),tp_ini(nmax)
       
      fbase = 'profile/profout'
      fname = 'profile/profout.s8'

cc      call mkfname(fname,fbase,imach,ioerr)

cc      call readcd(50,50,fbase,imach,nt,nz,ny,nx,nparam,ntimes,inf,ninf)
cc      call readd(50,u_ini,  1,1,ioerr)
cc      call readd(50,tp_ini, 1,2,ioerr)
cc      call readd(50,rho_ini,1,3,ioerr)
cc      call readd(50,v_ini,  1,4,ioerr) 
cc      close(50)
...
ccccc **********************************************************************
      SUBROUTINE ini_cond_blayer(local_imax,x,y,z)
