...
cccccc Version : 1.1
cccccc *******************************************************************
cccccc This program convert binary data for MATLAB file.
...
ccccc **********************************************************************
     ...
     &  U4 (imax,jmax,kmax),U5 (imax,jmax,kmax)
      ...
      open (1,file=dirgraf//'f_SN__showall.m',status='unknown',form='formatted') 
      do tt=t_ini,t_end
      ...
      do tt=t_ini,t_end
        if (mod(tt,qtimes).EQ.0) then
          ...
          if ((jmax.EQ.1).AND.(kmax.EQ.1)) then
            ...
            if (error.EQ.0) then
              call get_filename(tt,fname,'.m',fname_aux) 
