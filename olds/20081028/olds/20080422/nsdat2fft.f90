...
cc          call get_filename(tt,fnamegraf,'.dat',fname_aux)           
          ...
          tt_local=tt_local+1
          !MS$IF (tp_dim.EQ.2)
            call DFT_1D(tt_local,qtimes_local,M,v,freq_x,par_r,par_i,pwr_sp) 
          !MS$ELSEIF (tp_dim.EQ.3)
            call DFT_2D(tt_local,qtimes_local,M,N,v,freq_x,freq_z,par_r,par_i,pwr_sp) 
          !MS$ENDIF
      ...
      call write_intime(t_ini,t_end,qtimes_local,M,N,fname_time,pwr_sp)
...
ccccc **********************************************************************
      SUBROUTINE DFT_1D(tt_local,qtimes_local,M,par,freq_x,par_r,par_i,pwr_sp)
     ...
     &  par_r ((M-1)/2,1,qtimes_local),
     &  par_i ((M-1)/2,1,qtimes_local),
     &  pwr_sp((M-1)/2,1,qtimes_local)
      ...
      do i=1,(M-1)/2       
        freq_x(i           ) = dble(i-1  )
        par_r (i,1,tt_local) = data(i*2-1)
	par_i (i,1,tt_local) = data(i*2  )
	pwr_sp(i,1,tt_local) = 2.d0/dble(M-1)*Ma*c*
     &    dsqrt(par_r(i,1,tt_local)**2.d0+par_i(i,1,tt_local)**2.d0)        
      end do
...
ccccc **********************************************************************
      SUBROUTINE FFT_2D(tt_local,qtimes_local,M,N,par,freq_x,freq_z,
     &  par_r,par_i,pwr_sp)
     ...
     &  pwr_sp((M-1)/2,(N-1)/2,qtimes_local)
	  ...
	  pwr_sp(i,k,tt_local) = 2.d0/dble((M-1)*(N-1))*Ma*c*
...
ccccc **********************************************************************
      SUBROUTINE DFT_2D(tt_local,qtimes_local,M,N,par,freq_x,freq_z,
     &  par_r,par_i,pwr_sp)
...
ccccc **********************************************************************
ccccc write_intime: Write data for FFT modes in time.
ccccc **********************************************************************
      SUBROUTINE write_intime(t_ini,t_end,qtimes_local,M,N,fname,pwr_sp)
