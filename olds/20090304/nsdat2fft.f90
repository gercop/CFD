...
ccccc **********************************************************************
      SUBROUTINE dat2fft(t_ini,t_end,qtimes_local,N,fname)
      ...
      integer i,tt,tt_local,qtimes_local,t_ini,t_end,N,error
      real*8 x(imax),y(jmax),z(kmax),data(2*N),
     &  freq_x(N/2),pwr_sp(N/2,qtimes_local),
     &  par_r(N/2,qtimes_local),par_i(N/2,qtimes_local),
     ...
     &  U5 (imax,jmax,kmax)
      ...
      do tt=t_ini,t_end
        if (mod(tt,qtimes).EQ.0) then
          ...
          tt_local = tt_local + 1
          call fourier(tt_local,qtimes_local,N,data,freq_x,par_r,par_i,pwr_sp) 
      ...
      call write_FFT_t(t_ini,t_end,qtimes_local,N,
...
ccccc **********************************************************************
      SUBROUTINE fourier(tt_local,qtimes_local,N,data,freq_x,par_r,par_i,pwr_sp)

      IMPLICIT NONE
      INCLUDE 'nspar.f90'

      integer i,tt_local,qtimes_local,N            
      real*8 data(N*2),freq_x(N/2),
     &  par_r(N/2,qtimes_local),par_i(N/2,qtimes_local),
     &  pwr_sp(N/2,qtimes_local)
	...
	pwr_sp(i,tt_local) = Ma*c/2.d0*(2.d0/dble(N-1)*
...
ccccc **********************************************************************
     ...
     &  freq_x,par_r,par_i,pwr_sp)
      ...
      character*17 fname
      character*10 str
      character*17 headmodes
      character*1 NULLCHR
      integer i,tt,tt_local,qtimes_local,N,t_ini,t_end,size,modes,
...
cc        write(*,*) head
      ...
      IORE = TecIni('SIMPLE DATASET'//NULLCHR,'time m0 m1 m2 m3 m4 m5 m6 m7 m8 m9 m11 m12',
